...
def callback(message):
    ...
    for target_tran_pair in target_transforms_pairs:
        target_head_frame_id = "reference/" + target_tran_pair[0]
        target_child_frame_id = "reference/" + target_tran_pair[1]
        for tran in trans:
            header_frame_id = tran.header.frame_id
            child_frame_id = tran.child_frame_id
            #header_receive_time = tran.header.stamp
            if (header_frame_id == target_head_frame_id 
            	and child_frame_id == target_child_frame_id):
                ...
                print quaternion
                print '\n'
                key = target_tran_pair[0] + '_to_' + target_tran_pair[1]
                print key
